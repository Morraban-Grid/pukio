...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (user && user.rol === 'CAJERO') {
        // Ocultar Reportes para Cajeros (RF-50)
        const reportMenuItem = document.querySelector('.menu-item-reportes');
        if (reportMenuItem) {
            reportMenuItem.style.display = 'none';
        }
        // Ocultar Administración de Usuarios para Cajeros (RF-51 a RF-53)
        const usuariosMenuItem = document.querySelector('.menu-item-usuarios');
        if (usuariosMenuItem) {
            usuariosMenuItem.style.display = 'none';
        }
        // Redirigir si un cajero intenta acceder directamente a páginas restringidas
        const paginaActual = window.location.pathname;
        if (paginaActual.includes('reportes.html') || paginaActual.includes('usuarios.html')) {
